...
if __name__ == "__main__":
    ...
    ADMIX      = methfile.split('/')[-1].split('_')[0]
    B          = blockID.BlockID.unique()
    no_samples = methTable.shape[1]-1
    new_file   = "/isb/chumphri/Meth_" + ADMIX+ "_" + str(len(B)) + "_Block_Correlation_" + str(no_samples)+ "_samples.Regressed.tsv"

    with open(new_file, "w") as f:

        for block in B:
            BlockCpGSites = blockID[blockID['BlockID'] == block].Location
