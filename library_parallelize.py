...
if __name__ == '__main__':
    ...
    for num in xrange(1,51):
        stop  = lines*num
        file  = "/path/to/file/new/iteration_" + str(start) + "_" + str(stop) + ".txt"
        datFile += 1
        dat   = "/path/to/file/19K_meth_cellCorrected_101.txt" + str(datFile)
        p     = Process(target=run_main, args=(file, start, stop, dat))
        p.start()
        start = stop + 1
        print file
    ...
    cmd   = "cat " + files + " > Corected_CpG_values.txt" 
    os.system(cmd)
